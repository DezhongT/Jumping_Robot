...
def main():
    ...
    mu_t =  0.4231008938838332
    rho_t = 0.001292034620946721
    target_p = np.array([0.15070193, 0.81060987]).reshape(-1, 2)

    start_time = time.time()
    # let us try direct optimization
    points = np.hstack((x.reshape(-1, 1), y.reshape(-1, 1)))
    diff_p = points - target_p
    ...
    while y_g > 1e-3 or iter < 100:
        ...
        line_search_step = 0
        while True:
            alpha_t = alpha_g - alpha * jacobian[0]
            compressL_t = compressL_g - alpha * jacobian[1]
        ...
        alpha_g = alpha_g - alpha * jacobian[0]
        compressL_g = compressL_g - alpha * jacobian[1]
        y_g = y
